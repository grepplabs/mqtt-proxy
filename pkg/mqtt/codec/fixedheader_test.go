...
func TestFixedHeaderDecodeError(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			input:    "008080808001",
			expected: "the maximum number of bytes in the remaining length is 4, but was 5",
		},
		{
			name:     "remaining length too long",
			input:    "00808080808001",
			expected: "the maximum number of bytes in the remaining length is 4, but was 6",
		},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
